...
test_table<-read.csv(file="//filecs01/Newc/Location/File.csv")
...
#Add column names
colnames(test_table_melt)<-c("variable","value","MEGs","colorset")
...
plot_table <- function(df1,title1){
  p_out <- ggplot(data=df1,aes(x=variable,y=value)) +
              ...
              geom_point(data=test_table_melt[test_table_melt$value > test_table_melt$upper.limit | 
                          ...
                          aes(x=factor(variable), y=value, shape=factor(MEGs))) +
...
ptest <- plot_table(test_table_melt,"Top 30 MEG Allowed Per Episode Distribution")
ptest
pdf("Top 30 MEGs at Top 30 Systems.pdf", height=9, width=20)
print(ptest)
dev.off()