...
#Add column names
colnames(test_table_melt)<-c('variable','value','HCG.Detail','colorset')
...
plot_table <- function(df1,title1){
  p_out <- ggplot(data=df1,aes(x=variable,y=value)) +
              ...
              geom_point(data=test_table_melt[test_table_melt$value > test_table_melt$upper.limit | 
                          ...
                          aes(x=factor(variable), y=value, shape=factor(HCG.Detail))) +
...
ptest <- plot_table(test_table_melt,"Z scores by systems")
ptest
