...
Procedurestudy <- Procedure %>% 
  dplyr::select(proc_srv, time, time_c, billed_per_qty)
...
order_lm2<-length(lm_effects$proc_srv):1
...
decreasing_slope_lm <- lm_effects %>% 
  arrange(order_lm2) %>% .[["proc_srv"]]

Procedure$proc_srv <- factor(Procedure$proc_srv, 
...
for(i in 1:6){
Procedure3<-subset(Procedure, as.character(proc_srv)==as.character(top_lm$proc_srv[i]))
Procedure2<-bind_rows(Procedure2,Procedure3)
}

#Filter the NAs on Procedure2
Procedure2<-dplyr::filter(Procedure2, proc_srv !="NA")
...
base_plot <- base_plot +
  scale_x_continuous(breaks=cbind(12,35), labels=c("April 2013","March 2015")) 

base_plot$layers[[1]] <- geom_point(aes(color = factor(proc_srv), 
                                        size = 4))
...
subj_base_plot <- base_plot + facet_wrap("proc_srv", ncol = 2)+
...
decreasing_slope_lm <- lm_effects %>% arrange(order_lm2) %>% .[["proc_srv"]]

Procedure2$proc_srv <- factor(Procedure2$proc_srv,levels = decreasing_slope_lm)

PlotTopLM <- PlotTopLM +labs(x="Time in Months, 12 = April 2013, 35 = March 2015"
                             #, y="Billed Per Service Quantity"
                             ,y=element_blank()
                            )

PlotTopLM<- PlotTopLM + ggtitle(expression(atop("Outpatient Hospital Variation in Billed Per Quantity",atop(italic("Top 6 Trending Outpatient CPTs"), ""))))

PlotTopLM2<-PlotTopLM + 

                            theme(axis.text.x = element_text(angle=0, hjust=0.5, vjust=1, colour= "black", size = 15, face = "bold")
                             ...
                             ,axis.title.x = element_text(size = 25, angle = 00, face="bold")
